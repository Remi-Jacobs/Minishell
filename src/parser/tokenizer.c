...
void squish_args(t_shell_state *shell_state)
{
    ...
    while (token)
    {
        ...
        if (is_type(token, ARG) && is_types(prev, "TAI"))
        {
            ...
            token->prev = prev;
            token->next = (prev) ? prev->next : shell_state->cmd_list;
            prev = (prev) ? prev : token;
            prev->next->prev = token;
            prev->next = (shell_state->cmd_list->prev) ? prev->next : token;
            shell_state->cmd_list = (shell_state->cmd_list->prev) ? shell_state->cmd_list->prev : shell_state->cmd_list;
        }
        token = token->next;
    }
}
